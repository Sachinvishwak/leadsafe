...
class Api extends Common_Service_Controller{
    ...
    function userlogin_post(){
        ...
        if($this->form_validation->run() == FALSE)
        {
            ...
            $x = new stdClass();
            $result                 = $this->api_model->user_login($table_name,$data,$authtoken,$role,$foldername);
            if(is_array($result)){
                switch ($result['returnType']) {
                    case "SL":
                    $response = array('status' => SUCCESS, 'message' => ResponseMessages::getStatusCodeMessage(106), 'users' => $result['userInfo']);
                    break;
                    case "WP":
                    $response = array('status' => FAIL, 'message' => ResponseMessages::getStatusCodeMessage(102),'users'=>$x);
                    break;
                    case "WE":
                    $response = array('status' => FAIL, 'message' => ResponseMessages::getStatusCodeMessage(126),'users'=>$x);
                    break;
                    case "IU":
                    $response = array('status' => FAIL, 'message' => ResponseMessages::getStatusCodeMessage(118),'users'=>$x);
                    break;
                    case "WS":
                    $response = array('status' => FAIL, 'message' => ResponseMessages::getStatusCodeMessage(118),'users'=>$x);
                    break;
        ...
        if(isset($_FILES['licence_media']))
        {
			...
			if($this->upload->do_upload('filedata'))
			{
				...
				echo json_encode(array(
					'name' => $fileName,
					'file_path' => base_url('uploads/crew/').$licence,
					'status' => 'success',
