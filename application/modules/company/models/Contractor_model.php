...
class Contractor_model extends CI_Model {
    ...
    var $column_search = array('company_name','owner_first_name','owner_last_name'); //set column field database for datatable searchable 
    ...
    private function _get_query()
    {
        ...
        $this->db->select('company_member_relations.type, contractor.*')
                                ...
                                ->where('company_member_relations.type','leadcontractor')->order_by("contractor.id", "desc");
    ...
    {   
        $checkEmail = $this->db->select('*')->where(array('email'=>$user['email']))->get('contractor');
        if($checkEmail->num_rows()){
            $existId = $checkEmail->result();
            $existId = $existId[0]->id;
            return array('regType'=>'AE','existId'=>$existId);
        }else{
            $this->db->insert('contractor',$user);
            $lastId = $this->db->insert_id();
            if($lastId):
                $returnData = $this->db->get_where('contractor',array('id'=>$lastId))->result();
