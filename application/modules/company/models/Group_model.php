...
class Group_model extends CI_Model {
    ...
    var $table          = 'chat_group';
    var $column_order   = array('id','name','created_at','updated_at','status'); //set column field database for datatable orderable
    ...
    private function _get_query()
    {
		...
		$this->db->where(array('created_by'=>$_POST['id'])); 
    ...
    {   
        // $checkEmail = $this->db->select('*')->where(array('name'=>$user['name'],'created_by'=>$user['company_id'],'project_id'=>$user['project_id'))->get('chat_group');
        // if($checkEmail->num_rows()){
        //     return array('regType'=>'AE'); //already exist
        // }else{
            $this->db->insert('chat_group',$user);
            $lastId = $this->db->insert_id();
            if($lastId):
                $returnData = $this->db->get_where('chat_group',array('id'=>$lastId))->result();
