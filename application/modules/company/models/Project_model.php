...
class Project_model extends CI_Model {
    ...
    var $table          = 'project';
    var $column_order   = array('id','name','status','created_at'); //set column field database for datatable orderable
    
    var $column_sel     = array('*',
        'status',
        '(case when (status = 0) 
        THEN "Open Project" when (status = 1) 
        THEN "Close Project" ELSE
        "Unknown" 
        END) as statusShow'); 
    ...
    {   
        $checkEmail = $this->db->select('*')->where(array('name'=>$user['name'],'company_id'=>$user['company_id']))->get('project');
        if($checkEmail->num_rows()){
            ...
            if($lastId):
                $returnData = $this->db->get_where('project',array('id'=>$lastId))->result();
